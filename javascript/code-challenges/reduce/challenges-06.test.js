...
const reversedString = (str) => {
  let a = str.split("").reduce((a, v) => v+a, "")
  return a
};
...
const extractStats = (arr) => {
  return arr.stats.reduce((a,v)=>{
    a[v.stat.name] = v.baseStat
    return a
  }, {})
};
...
const extractChildren = (arr) => {
  // Solution code here...
  return arr.filter(x => x.name.includes('a') && 'children' in x).reduce((a, v)=>[...a, ...v.children],[])
};
