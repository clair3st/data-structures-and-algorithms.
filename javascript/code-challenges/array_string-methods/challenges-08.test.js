...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(x=>{
    let b = x.slice(x.indexOf(' ')+1)
    result.push(b.slice(b.indexOf(' ')+1))
  })
  return result;
};
...
const splitFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach(x=>{
    result.push(x.split(' ').slice(2,).join(' '))
  })
  return result;
};
...
const removeLastCharacters = (str, n) => {
  if (n > str.length) return ''
  if (n < 0) return str
  return str.slice(0, name.length - n)
};
